...
export const devServerConfig = (params: ConfigParams): WebpackConfiguration => ({
  ...
  devServer: {
    ...
    client: {
      // Firefox reports cancelled worker script loads as "NetworkError: A network error occurred".
      // In this local viewer workflow the dev-server overlay can block the app even though the
      // bundle compiled successfully, so keep diagnostics in the terminal/browser console instead.
      overlay: false,
    },
  },
